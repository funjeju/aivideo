...
- [ ] **★업소 사진 → 화풍 변환 후 장면 배치 (시장성 변수 — 우선순위 높음)**
  - 아이디어: 식당/카페 등 업체가 **실제 사진(외관·메뉴·내부)을 첨부** → 원고에 따라 장면 이미지 만들 때, 해당 사진을 **원하는 화풍으로 변환(`gpt-image-2` images.edit)** 해서 그 장면에 배치. 사명/로고도 같이 반영.
  - 실현성 ★높음: 지금 업소용이 이미 로고를 매 장면 `images.edit` reference로 넣는 구조 → 사진을 소스로 넣고 "이 화풍으로 변환" 프롬프트만 추가. 멀티 reference(사진+로고) 동시 가능.
  - **선행(리스크 0)**: 어드민 업소용 테스트에 "사진 1장 → 화풍 변환" 단건 붙여 **품질 + 실제 응답 usage 토큰 실측** 먼저. 좋으면 create 업소용 탭에 사진 업로드 + 장면↔사진 매칭(AI 자동 or 수동) 추가.
  - **비용**(검색 확인, 2026.6): generate=출력토큰만, edit=출력+**reference 입력토큰($8/1M, 항상 high-fidelity)**. low 출력 기준 edit 장면은 ≈2배. 2분(17장면) 전부 low일 때 edit 0%=$0.34 vs edit 50%≈$0.52(+53%). **반복 로고/사진은 캐싱($2/1M, 1/4)되면 50% edit이 ≈$0.39(+15%)로 감소**. 풀변환도 1분 ~$1 이내.
  - 주의: 변환 장면 vs 순수 AI생성 장면 **화풍 일관성**, 사진 품질 의존, 매칭에 Vision 1회 추가.
