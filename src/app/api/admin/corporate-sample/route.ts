...
export async function POST(req: NextRequest) {
  ...
  try {
    const { subject, companyKo, companyEn, quality, aspect, stylePackId, logoDataUrl, useLogoRef } = await req.json();
    ...
    const prompt = pack.imagePrompt.template.replace("{subject}", String(subject).trim()) + brandInstr + logoInstr;

    let b64: string | undefined;
    if (willUseLogo) {
      // 로고를 reference로 넣어 edit — 로고 반영 시도
      const buf = Buffer.from(logoDataUrl.split(",")[1] ?? "", "base64");
      const file = await toFile(buf, "logo.png", { type: "image/png" });
      const r = await openai.images.edit({ model: "gpt-image-2", image: file, prompt, size, quality: q });
    ...
    return NextResponse.json({ image: `data:image/png;base64,${b64}`, prompt, quality: q, usedLogo: willUseLogo });
  } catch (e) {
