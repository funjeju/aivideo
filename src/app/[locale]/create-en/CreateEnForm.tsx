...
// 보이스 = 영문 보이스만 필터링
const EN_VOICE_LIST = VOICES.filter(v => v.languageCode === "en-US").map((v) => ({ id: v.id, name: v.name, previewUrl: voicePreviewUrl(v.id) }));

export default function CreateEnForm() {
  ...
  const [voiceId, setVoiceId] = useState(EN_VOICE_LIST[0]?.id || "nova");
  const voices = EN_VOICE_LIST;
  ...
  async function startGeneration() {
    ...
    try {
      ...
      const scriptRes = await fetch("/api/script", {
        ...
        body: JSON.stringify({ projectId, mode: submitMode, topic, targetLength, contentLocale: "en" }),
      });
  ...
  return (
    <main className="flex-1 p-6 max-w-2xl mx-auto w-full">
      <h1 className="text-2xl font-semibold text-[var(--ink)] mb-8">영어영상 만들기 (English)</h1>
