...
- **나레이션은 음성합성(TTS)으로 읽힌다 — 성우가 자연스럽게 끊어 읽도록 호흡 지점에 쉼표를 넣어라.** 긴 수식어구 뒤, 접속어(그래서·하지만·그러나·즉) 앞뒤, 주어가 길 때 주어 뒤에 쉼표(,)로 끊어줘라. 단, 문어체로 어색해질 만큼 과하게 찍지는 말고 자연스러운 구어 호흡만큼만. 어절 띄어쓰기는 표준 맞춤법을 정확히 지켜라(붙여쓰기/오타 금지 — TTS가 끊어읽기를 띄어쓰기로 판단한다).
